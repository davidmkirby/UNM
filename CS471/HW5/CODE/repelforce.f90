...
! bbar is the average slope 
     subroutine b_bar(bt,c1t,Bbar,Bbar1,repel,smelly,mu)
     implicit none
     real(kind=8),intent(in)::bt(1:nbirds) 
     real(kind=8),intent(in)::c1t,repel,smelly,mu
     integer::i     
     ...
     do i = 2,nbirds
       Bbar= Bbar + bt(i)
       Bbar1 = Bbar1 + mf(bt(1),bt(i),c1t,Bbar,Bbar1,repel,smelly,mu)
     end do
     Bbar = Bbar/nbirds
     Bbar1 = Bbar1/nbirds
...
!runge-kutta method 
    real(kind=8) function mf(bt1,bkt,c1t,Bbar,Bbar1,repel,smelly,mu)
    implicit none
    real(kind=8),intent(in):: bt1,bkt,c1t
    real(kind=8),intent(in):: Bbar,Bbar1,repel,smelly,mu
    real(kind=8):: k11,a1,ak,ad,k22,b1,bk,bd,k33,c1,ck,cd,k44
    k11 = gamma2*(bt1-bkt) + kappa*(Bbar-bkt) + repel + mu*(bkt-smelly)
    a1 = bt1 + kf(c1t,bt1)*dt/2.d0
    ak = bkt+ k11*dt/2.d0
    ad = Bbar + Bbar1*dt/2.d0
    k22 = gamma2*(a1 - ak) + kappa*(ad-ak) + repel + mu*(ak - smelly)
    b1 = bt1 + kf(c1t,bt1)*dt/2.d0
    bk = bkt+ k22*dt/2.d0
    bd = Bbar + Bbar1*dt/2.d0
    k33 = gamma2*(b1 - bk) + kappa*(bd-bk) +repel + mu*(bk-smelly)
    c1 = bt1 + kf(c1t,bt1)*dt
    ck = bkt+ k33*dt
    cd = Bbar + Bbar1*dt
    k44 = gamma2*(c1 - ck) + kappa*(cd-ck)+repel + mu*(ck-smelly)
