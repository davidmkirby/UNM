...
implicit none
  ...
  real(kind=8)::x,y
  real(kind=8) :: smelly_x,smelly_y,mu
  real(kind=8) :: repel
...
! xy-position evolution for leader
 ...
 !position evolution of remaining birds
     do j=1,nbirds
      ...
      write(1,*) smelly_x,smelly_y

      do j=2,nbirds
         call repels(b1(:),B_x(j,i-1),repel)
         B_x(j,i) = B_x(j,i-1) + dt*mf(B_x(1,i-1),B_x(j,i-1),C1_x(i-1),Bbarx,Bbarx1,repel,smelly_x,mu)
         call repels(b1(:),B_y(j,i-1),repel)
         B_y(j,i) = B_y(j,i-1) + dt*mf(B_y(1,i-1),B_y(j,i-1),C1_y(i-1),Bbary,Bbary1,repel,smelly_y,mu)
        
      end do
      
      call b_bar(b1,C1_x(0),Bbar,Bbar1,repel,smelly_x,mu)
      Bbarx = Bbar
      Bbarx1 = Bbar1

      call b_bar(b2,C1_y(0),Bbar,Bbar1,repel,smelly_y,mu)
      Bbary = Bbar 
      Bbary1 = Bbar1
